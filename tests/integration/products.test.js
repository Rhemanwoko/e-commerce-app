...
describe("Product Endpoints", () => {
  let adminUser, customerUser, adminToken, customerToken, testBrand;
  ...
  beforeEach(async () => {
    ...
    // Create test brand
    testBrand = new Brand({ brandName: "Test Brand" });
  ...
  describe("GET /products", () => {
    ...
    it("should get all products without authentication", async () => {
      ...
      expect(response.body.data.products[0].productName).toBe(
    ...
    it("should populate owner information in product list", async () => {
      ...
      expect(response.body.data.products[0].ownerId.fullName).toBe(
        adminUser.fullName
      );
      expect(response.body.data.products[0].ownerId.email).toBe(
        adminUser.email
      );
  ...
  describe("POST /products", () => {
    ...
    it("should create product with admin authentication", async () => {
      ...
      expect(response.body.data.product.productName).toBe(
        validProductData.productName
      );
      ...
      expect(response.body.data.product.ownerId.fullName).toBe(
        adminUser.fullName
      );
    });
    ...
    it("should fail validation when brand ID is invalid", async () => {
      const productWithInvalidBrand = {
        ...validProductData,
        brand: "invalid-brand-id",
      };

      const response = await request(app)
        .post("/products")
        .set("Authorization", `Bearer ${adminToken}`)
        .send(productWithInvalidBrand)
        .expect(400);
    ...
    it("should fail when brand does not exist", async () => {
      const nonExistentBrandId = new mongoose.Types.ObjectId();
      const productWithNonExistentBrand = {
        ...validProductData,
      ...
      const response = await request(app)
        .post("/products")
        .set("Authorization", `Bearer ${adminToken}`)
        .send(productWithNonExistentBrand)
        .expect(400);
    ...
    it("should populate brand information in created product", async () => {
      const response = await request(app)
        .post("/products")
        .set("Authorization", `Bearer ${adminToken}`)
        .send(validProductData)
      ...
      expect(response.body.data.product.brand).toBeDefined();
      expect(response.body.data.product.brand._id).toBe(
        testBrand._id.toString()
      );
      expect(response.body.data.product.brand.brandName).toBe("Test Brand");
    });
  });
  ...
  describe("DELETE /products/:id", () => {
    ...
    it("should delete product with admin authentication", async () => {
      ...
      const getResponse = await request(app).get("/products").expect(200);
  ...
  describe("Product Endpoints Integration", () => {
    it("should complete full product lifecycle (create, read, delete)", async () => {
      ...
      const readResponse = await request(app).get("/products").expect(200);
      ...
      const finalReadResponse = await request(app).get("/products").expect(200);
    ...
    it("should handle multiple products from different admins", async () => {
      ...
      const productNames = response.body.data.products.map(
        (p) => p.productName
      );
